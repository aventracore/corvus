  ...
  return (
    <section className="container container-gutter py-16 md:py-24">
      ...
      <div className="mt-8 flex items-center justify-center gap-4">
        ...
        <input id="seats" type="range" min={1} max={50} value={seats} onChange={(e) => setSeats(parseInt(e.target.value))} />
        <span className="text-sm font-semibold tabular-nums">{seats}</span>
      </div>

      <div className="mt-10 grid gap-6 md:grid-cols-3">
        {tiers.map((t) => {
          const price = computePrice(t.baseMonthly, seats, annual);
          return (
            <div key={t.name} className={`rounded-2xl border p-6 ${t.highlight ? 'border-brand-500 shadow-glow bg-white' : 'border-black/10 dark:border-white/10 bg-white/60 dark:bg-black/30 backdrop-blur'}`}>
              <h3 className="text-lg font-semibold">{t.name}</h3>
              <p className="mt-6 text-3xl font-bold tabular-nums">${price}<span className="text-base font-normal text-black/60 dark:text-white/60">/{annual ? 'yr' : 'mo'}</span></p>
